...
int main (int argc, char **argv) {
	...
	double min[IdxInv.GetSize()], max[IdxInv.GetSize()];
	int nbits[IdxInv.GetSize()];
	...
	if ( file != NULL ) {
		// Output file
		ofstream myfile;
		myfile.open("nsgadata/NSGA_summary.csv");
		...
		for (;;) {
			...
			for (int i = 0; i < IdxInv.GetSize(); ++i) {
				...
				double lbValue = min[i] + (max[i]-min[i]) * actual / maxim;
				netplan.var[0][inv + i].setLB( lbValue );
			}
