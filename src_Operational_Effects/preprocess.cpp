...
int main() {
	...
	for (unsigned int k=0; k < ListNodes.size(); ++k) {
		...
		if ( ListNodes[k].Get("Step") == "") {
			...
			while ( TempStep <= SLength ) {
				...
				// Calculate contingency reserve for a year - Venkat Krishnan May 19 -- only for operational
				/* if (TempNode.Get("ContingencyReserve") != "X") {
					double step_length = TempNode.GetDouble("StepLength");
					TempNode.Multiply("ContingencyReserve", step_length);
				} */

				// Calculate Regulation (load) for a year - Venkat Krishnan May 28 // here INV
				if (TempNode.Get("Regulation") != "X") { // check regulation value of load????
					double NLoad = sqrt(TempNode.GetDouble("Demand")/(TempNode.GetDouble("StepLength")*Lbase));// ith year demand
					double RegL = (TempNode.GetDouble("Regulation")/100)*Lbase;
					TempNode.Set("Regulation", ToString<double>(NLoad*RegL));// N ---> (N+d/2*N) --> square root
					Regvalue.push_back(NLoad*RegL);
					Regnode.push_back(TempNode.Get("Code"));
				} 

				// Calculate Ramp 10 min. (load) for a year - Venkat Krishnan June 28 // here INV
				if (TempNode.Get("Ramp10") != "X") { 
					double NLoad = sqrt(TempNode.GetDouble("Demand")/(TempNode.GetDouble("StepLength")*Lbase));// ith year demand
					double RegL10 = (TempNode.GetDouble("Ramp10")/100)*Lbase;
					TempNode.Set("Ramp10", ToString<double>(NLoad*RegL10));// N ---> (N+d/2*N) --> square root
					Regvalue10.push_back(NLoad*RegL10);
					Regnode10.push_back(TempNode.Get("Code"));
				} 

				// Calculate Ramp 30 min. (load) for a year - Venkat Krishnan June 28 // here INV
				if (TempNode.Get("Ramp30") != "X") { 
					double NLoad = sqrt(TempNode.GetDouble("Demand")/(TempNode.GetDouble("StepLength")*Lbase));// ith year demand
					double RegL30 = (TempNode.GetDouble("Ramp30")/100)*Lbase;
					TempNode.Set("Ramp30", ToString<double>(NLoad*RegL30));// N ---> (N+d/2*N) --> square root
					Regvalue30.push_back(NLoad*RegL30);
					Regnode30.push_back(TempNode.Get("Code"));
				}


				// Calculate Ramp 60 min. (load) for a year - Venkat Krishnan June 28 // here INV
				if (TempNode.Get("Ramp60") != "X") { 
					double NLoad = sqrt(TempNode.GetDouble("Demand")/(TempNode.GetDouble("StepLength")*Lbase));// ith year demand
					double RegL60 = (TempNode.GetDouble("Ramp60")/100)*Lbase;
					TempNode.Set("Ramp60", ToString<double>(NLoad*RegL60));// N ---> (N+d/2*N) --> square root
					Regvalue60.push_back(NLoad*RegL60);
					Regnode60.push_back(TempNode.Get("Code"));
				}
				...
				if ( (TempNode.Get("PeakPower") != "X") && TempNode.isFirstinYear() ) {
					...
					temp2[0] = TempStep[0];
					IdxRm.Add(k, temp2, TempNode.Get("ShortCode")); // Reserve Margins
					IdxRmrc.Add(k, temp2, TempNode.Get("ShortCode")); // INV June 22 Ramp 1 -min
					IdxRmrp10.Add(k, temp2, TempNode.Get("ShortCode")); // INV June 29 Ramp 10 -min
					IdxRmrp30.Add(k, temp2, TempNode.Get("ShortCode")); // INV June 29 Ramp 30 -min
					IdxRmrp60.Add(k, temp2, TempNode.Get("ShortCode")); // INV June 29 Ramp 60 -min
				}
	...
	// May 28 - Writing Regulation into file - Venkat
	WriteOutput("prepdata/AreaRegulation.csv", Regnode, Regvalue, "const");
	WriteOutput("prepdata/AreaRamp10.csv", Regnode10, Regvalue10, "const");
	WriteOutput("prepdata/AreaRamp30.csv", Regnode30, Regvalue30, "const");
	WriteOutput("prepdata/AreaRamp60.csv", Regnode60, Regvalue60, "const");
	...
	for (unsigned int k = 0; k < ListArcs.size(); ++k) {
		...
		if ((ListArcs[k].Get("FromStep") == "") && (ListArcs[k].Get("ToStep") == "")) {
			...
			while ((TempStep <= SLength) && ( TempToStep <= SLength)) {
				...
				double op_cost = TempArc.GetDouble("OpCost"); // Venkat
				//double op_cost = TempArc.GetDouble("InvCost");

				if ( factor != 1 && ( inv_cost != 0 || op_cost != 0 ) ) { // test	
				...
				// May 23 2011 - Venkat Krishnan - Index file for CR
					if (TempArc.Get("RampRate")!="0"){ 
						IdxCR.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To"));
						IdxCRop.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To")); // June 05 2012
						IdxRU.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To")); // May 27
						IdxRD.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To"));
						//IdxSR.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To")); // July 4
						//IdxRR.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To"));						
					}
					/*if (TempArc.Get("Cycles")!="0"){ // Aug 11 Cycling
						IdxCyE.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To"));
						IdxCyCR.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To"));
						IdxCyRU.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To"));
						IdxCyRD.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To"));
						IdxCySR.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To"));
						IdxCyRR.Add(k, TempStep, TempArc.Get("From") + "_" + TempArc.Get("To"));
					}*/

					if (TempArc.Get("RampRate")!="0"){ // INV June 29 Ramps
						if ( TempArc.isFirstinYear() ) {
 							Step temprc( SName.size(), 0 );
							temprc[0] = TempStep[0];
							IdxRC10.Add(k, temprc, TempArc.Get("From") + "_" + TempArc.Get("To"));
							IdxRC30.Add(k, temprc, TempArc.Get("From") + "_" + TempArc.Get("To"));
							IdxRC60.Add(k, temprc, TempArc.Get("From") + "_" + TempArc.Get("To"));
						}
					}
	...
	ofstream afile; //, myfile[nyears+1]; // No Benders - AUg 15 2011 Venkat
...
/*	for (int i = 0; i <= nyears; ++i) {
	...
	// Nodal demand constraints //May 19 - addedd reserve - Venkat Krishnan; May 27 added Reg
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
	...
	// Upper bound constraints rows// May 19 - addedd reserve - Venkat Krishnan; May 27 added Reg // Aug 10 2011 - cycling added
	...
	// Reserve margin for ramp // INV
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].NodeRMrcColumns();
		afile << temp_string;
		//myfile[0] << temp_string;
	}

	// Reserve margin for ramp // INV June 29 10 min. Ramp
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].NodeRMrp10Columns();
		afile << temp_string;
		//myfile[0] << temp_string;
	}

	// Reserve margin for ramp // INV June 29 30 min. Ramp
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].NodeRMrp30Columns();
		afile << temp_string;
		//myfile[0] << temp_string;
	}

	// Reserve margin for ramp // INV June 29 60 min. Ramp
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].NodeRMrp60Columns();
		afile << temp_string;
		//myfile[0] << temp_string;
	}
	...
	// Flows
	for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcColumns();// May 20 Venkat krishnan --> Contin reserve + energy + Reg (May 27)
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}
	...
	// Ramp capacity 10 min.// INV June 29
	for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcRamp10Columns();
		afile << temp_string;
		//myfile[0] << temp_string;
	}

	// Ramp capacity 30 min. // INV June 29
	for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcRamp30Columns();
		afile << temp_string;
		//myfile[0] << temp_string;
	}

	// Ramp capacity 60 min. // INV June 29
	for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcRamp60Columns();
		afile << temp_string;	
		//myfile[0] << temp_string;
	}
	...
	// Contingency Reserve Req. within region - // May 20 Venkat Krishnan
	for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcCRColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}

	// Contingency Reserve operational within region - // June 05 2012 Venkat Krishnan
	for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcCRopColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}

	// Regulation Up Req. within region - // May 27 Venkat Krishnan
	for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcRUColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}

	// Regulation Down Req. within region - // May 27 Venkat Krishnan
	for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcRDColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}


	// Spinning Reserve Req. within region - // July 4 Venkat Krishnan
	/*for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcSRColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}*/


	// Replacement Reserve Req. within region - // July 4 Venkat Krishnan
	/*for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcRRColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}*/

	// Cycling Aug 10 2011 Venkat Krishnan - Energy
	/*for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcCyEColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}*/

	// Cycling Aug 10 2011 Venkat Krishnan - Contingency Res
	/*for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcCyCRColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}*/

	// Cycling Aug 10 2011 Venkat Krishnan - Regu Up
	/*for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcCyRUColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}*/

	// Cycling Aug 10 2011 Venkat Krishnan - Regu Down
	/*for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcCyRDColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}*/

	// Cycling Aug 10 2011 Venkat Krishnan - Spinning
	/*for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcCySRColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}*/

	// Cycling Aug 10 2011 Venkat Krishnan - Replacement
	/*for (unsigned int i = 0; i < Arcs.size(); ++i) {
		temp_string = Arcs[i].ArcCyRRColumns();
		afile << temp_string;
		//myfile[ Arcs[i].Time() ] << temp_string;
	}*/
	...
	// Nodal Contingency Reserve - May 20 Venkat Krishnan
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].CRRhs();
		afile << temp_string;
		//myfile[ Nodes[i].Time() ] << temp_string;
	} 

	// Nodal Regulation - May 27 Venkat Krishnan INV
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].RegRhs();
		afile << temp_string;
		//myfile[ Nodes[i].Time() ] << temp_string;
	}


	// Nodal Replacement Reserve - May 20 Venkat Krishnan
	/*for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].RRRhs();
		afile << temp_string;
		//myfile[ Nodes[i].Time() ] << temp_string;
	} */

	// Nodal Spinning Reserve - May 20 Venkat Krishnan
	/*for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].SRRhs();
		afile << temp_string;
		//myfile[ Nodes[i].Time() ] << temp_string;
	} */
	...
	// Peak ramp must be met // INV June 22
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].NodeRMrcBounds();
		afile << temp_string;
		//myfile[0] << temp_string;
	}

	// Peak 10 min. ramp must be met // INV June 29
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].NodeRMrp10Bounds();
		afile << temp_string;
		//myfile[0] << temp_string;
	}

	// Peak 30 min. ramp must be met // INV June 29
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].NodeRMrp30Bounds();
		afile << temp_string;
		//myfile[0] << temp_string;
	}

	// Peak 60 min. ramp must be met // INV June 29
	for (unsigned int i = 0; i < Nodes.size(); ++i) {
		temp_string = Nodes[i].NodeRMrp60Bounds();
		afile << temp_string;
		//myfile[0] << temp_string;
	}
	...
	IdxRmrc.WriteFile("prepdata/idx_rmrc.csv"); // INV June 22 new variable for ramp reserve
	...
	// write for new variables
	IdxCR.WriteFile("prepdata/idx_cr.csv"); // - May 23 Venkat Krishnan
	IdxCRop.WriteFile("prepdata/idx_crop.csv"); // - June 05 2012 - CRop Venkat Krishnan
	IdxRU.WriteFile("prepdata/idx_ru.csv"); // - May 27 Venkat Krishnan
	IdxRD.WriteFile("prepdata/idx_rd.csv"); // - May 27 Venkat Krishnan
	//IdxSR.WriteFile("prepdata/idx_sr.csv"); // - July 4 Venkat Krishnan
	//IdxRR.WriteFile("prepdata/idx_rr.csv"); // - July 4 Venkat Krishnan
 
	
	IdxRC10.WriteFile("prepdata/idx_rc10.csv"); // INV June 29 new variable for ramp
	IdxRC30.WriteFile("prepdata/idx_rc30.csv"); // INV June 29 new variable for ramp
	IdxRC60.WriteFile("prepdata/idx_rc60.csv"); // INV June 29 new variable for ramp
	IdxRmrp10.WriteFile("prepdata/idx_rmrp10.csv"); // INV June 29 new variable for 10 min. ramp reserve
	IdxRmrp30.WriteFile("prepdata/idx_rmrp30.csv"); // INV June 29 new variable for 30 min. ramp reserve
	IdxRmrp60.WriteFile("prepdata/idx_rmrp60.csv"); // INV June 29 new variable for 60 min. ramp reserve

	/*IdxCyE.WriteFile("prepdata/idx_cye.csv"); // Aug 11 cycling
	IdxCyCR.WriteFile("prepdata/idx_cycr.csv"); // Aug 11 cycling
	IdxCyRU.WriteFile("prepdata/idx_cyru.csv"); // Aug 11 cycling
	IdxCyRD.WriteFile("prepdata/idx_cyrd.csv"); // Aug 11 cycling
	IdxCySR.WriteFile("prepdata/idx_cysr.csv"); // Aug 11 cycling
	IdxCyRR.WriteFile("prepdata/idx_cyrr.csv"); // Aug 11 cycling*/
