...
string Node::NodeNames() const {
	...
	if ((Get("Demand") != "X") && (Get("Code")[0] != 'X')) {
		...
		//	for (int endprimy = 2; endprimy <= SLengthend; ++endprimy) {
		//		temp_output += " E " + Get("Code") + ToString<int>(endprimy) + "\n";
		//	}
		//}

		if (Get("ShortCode").substr(0,2)=="EC")
			temp_output += " L co" + Get("Code") + "\n"; // Mar 04 2014 co-fire
...
string Node::NodeUDColumns() const {
	...
	if (Get("CostUD") != "X") {
		...
		if (useEndPrimE && Time()==SLength[0]) {// End Effects April 29 2013
			temp_output += "    UD_" + Get("Code") + " obj " + ToString<double>(GetDouble("CostUD")/(1-Loadgrowth*alpha)) + "\n";
		}	else {
			if (useEndDualE && Time()==SLength[0]) // End Effects April 29 2013
				temp_output += "    UD_" + Get("Code") + " obj " + ToString<double>(GetDouble("CostUD")/(1-alpha)) + "\n";
			else	{
				temp_output += "    UD_" + Get("Code") + " obj " + Get("CostUD") + "\n";
				if (Time()<=Sobjeval)
					temp_output += "    UD_" + Get("Code") + " HTobj " + Get("CostUD") + "\n";
			}
		}
