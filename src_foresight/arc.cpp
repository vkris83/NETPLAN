...
string Arc::ArcCapNames() const {
	...
	if (isFirstinYear() && isTransport()  && Get("TransInfr") == "") {
		...
		if (Get("OpMax") != "Inf") {
			...
			if (useEndFixed && Time()>=SLength[0]-SLengthend+1) // End effect May 2
				temp_output += " E eqcap" + Get("Code") + "\n";

			if (useEndPrimE && Time()==SLength[0]) { // End Effects April 29 2013
				if (Get("LifeSpan") != "X") {
					for (int endprimy = 2; endprimy <= min(Str2Step(Get("LifeSpan"))[0],SLengthend); ++endprimy) {
						temp_output += " E inv2cap" + Get("Code") + ToString<int>(endprimy) + "\n";
					}
				}
			}
		}
...
string Arc::ArcColumns() const {
	...
	if (!isTransport()) {
		...
		if (Get("OpMax") != "Inf") {
			...
			// April 27 2012 - Bound on infratsructure - freight truck and car infra sharing... new Venkat - Transportation
...
string Arc::InvArcColumns() const {
	...
	if (InvArc() && Get("TransInfr") == "") {
		...
		if (Time()<=Sobjeval) {
			if (useEndSalvg) { // End Effect April 18 2013
				// Need to re-adjust for investment costs at the end of the simulation period- salvage value
				string life_span = Get("LifeSpan");
				if (life_span != "X") {
					int years_left = (SLength[0] + 1) - Time();
					int life_inv = Str2Step(life_span)[0];
					if (years_left < life_inv) 
						temp_output += "    inv" + Get("Code") + " HTobj " + ToString<double>(GetDouble("InvCost")* life_inv / years_left) + "\n";
				}
			} 
			else	temp_output += "    inv" + Get("Code") + " HTobj " + Get("InvCost") + "\n";
		}
		...
		step1 = StepSum(step1, Str2Step(Get("Leadtime")));
		step2 = StepSum(step2, Str2Step(Get("Leadtime")));
		if (Get("LifeSpan") != "X") {
			maxstep = StepSum(step1, Str2Step(Get("LifeSpan")));
			if (maxstep >= SLength) // End effect lifespan - April 26 2013
				life_more = maxstep[0]-(SLength[0] + 1) ;
			maxstep = (maxstep > SLength) ? SLength : maxstep;			
		} else {
			maxstep = SLength;
		}
		int endt=0; // end effect year - april 21 2013
		...
		while (stepguide <= maxstep) {
			...
			if (stepguide == SLength) {// end effect year - april 21 2013
				if (useEndDualE) {
					double alpha = (1 + GetDouble("InflationRate")) / (1 + GetDouble("DiscountRate"));
					double betam = 1, betas=0;
					if (endt==1) { // A matrix
						for (int endy = 1; endy <= SLengthend; ++endy) {
							betam = betam * alpha;
							betas=betas+betam;
						}
						temp_output += " -" + ToString<double>(1+betas) + "\n";
					}	else	{ // H matrix						
						for (int endy = 1; endy <= min(life_more-1, SLengthend-1); ++endy) {
							betam = betam * alpha;
							betas=betas+betam;
						}
						temp_output += " -" + ToString<double>((1+betas)*(1-alpha)) + "\n";
					}
				}	else if (useEndPrimE) {
					temp_output += " -1\n";					
					if (endt==1) { // A matrix
						for (int endy = 2; endy <= min(life_more+1, SLengthend); ++endy) {
							double betam = 1, betas=0;
							temp_output += "    inv" + Get("Code") + " inv2cap";
							temp_output += Get("From") + Step2Str(step1);
							if (!isTransport()) {
							...
							temp_output += ToString<int>(endy);
							for (int endpry = 1; endpry <= endy-1; ++endpry) {
								betam = pow(Loadgrowth,endy-endpry-1);
								betas=betas+betam;
							}
							temp_output += " -" + ToString<double>(pow(Loadgrowth,endy-1)+betas) + "\n";
						}
					}	else	{ // H matrix						
						for (int endy = 2; endy <= min(life_more+1, SLengthend); ++endy) {
							temp_output += "    inv" + Get("Code") + " inv2cap";
	...
	if (isFirstinYear() && Get("OpMax") != "Inf" && Get("TransInfr") == "") {
		...
		if (selector != 2) {
			...
			if (useEndPrimE && Time()==SLength[0]) { // End Effects April 29 2013
				if (Get("LifeSpan") != "X") {
					for (int endprimy = 2; endprimy <= min(Str2Step(Get("LifeSpan"))[0],SLengthend); ++endprimy) {
						temp_output += "    cap" + Get("Code") + " inv2cap" + Get("Code") + ToString<int>(endprimy) +" "+ ToString<double>(pow(Loadgrowth,endprimy-1)) +"\n";						
					}
				}
			}
			
			if (useEndFixed)	{// End effect May 2
				if (Time()>=SLength[0]-SLengthend+1)
					temp_output += "    cap" + Get("Code") + " eqcap" + Get("Code") + " 1\n";
				if (Time()==SLength[0]-SLengthend)	{
					Step step1, step2;
					step1 = Str2Step(Get("FromStep"));
					step2 = Str2Step(Get("ToStep"));
					for (int endfixedy = 1; endfixedy <= SLengthend; ++endfixedy) {
						temp_output += "    cap" + Get("Code") + " eqcap" + Get("From") + Step2Str(NextStep(step1));
						if (!isTransport()) {
							temp_output += "_" + Get("To") + Step2Str(NextStep(step2));
						}
						temp_output +=	" -1\n";
				
						if (NextStep(step1) <= SLength)
							step1 = NextStep(step1);
						if (NextStep(step2) <= SLength)
							step2 = NextStep(step2);
					}
				}
			}
...
string Arc::ArcRhs() const {
	...
	if (isFirstinYear() && (Get("OpMax") != "Inf")  && (Get("TransInfr") == "")) {
		...
		if (useEndPrimE && Time()==SLength[0]) { // End Effects April 29 2013
			if (Get("LifeSpan") != "X") {
				for (int endprimy = 2; endprimy <= min(Str2Step(Get("LifeSpan"))[0],SLengthend); ++endprimy) {
					temp_output += " rhs inv2cap" + Get("Code") +ToString<int>(endprimy) + " " + ToString<double>(GetDouble("OpMax")*pow(CapRed,endprimy-1)) + "\n";
				}
			}
		}
	}
