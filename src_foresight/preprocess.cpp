...
int main() {
		...
		if (ListNodes[k].Get("Step") == "") {
			...
			while (TempStep <= SLength) {
				...
				if ((dem_rate != 0) || (peak_rate != 0)) {
					if (useEndFixed) { // End Effect May 03 2013
						for (unsigned int l = 1; l < min(SLength[0]-SLengthend, TempStep[0]); ++l)	{
					...
					// April 18 2013 End Effect- dual eq.
					double betam = 1, betasd=dem_factor, betasp=peak_factor;
					if (useEndDualE && (TempStep[0] == SLength[0])) {
						double alpha = (1 + TempNode.GetDouble("InflationRate")) / (1 + TempNode.GetDouble("DiscountRate"));						
						for (int endy = 1; endy <= SLengthend-1; ++endy) {
							dem_factor = dem_factor * (1 + dem_rate);
							peak_factor = peak_factor * (1 + peak_rate);
							betam = betam * alpha;
							betasd=betasd+betam*dem_factor; 
							betasp=betasp+betam*peak_factor; 
						}
							TempNode.Multiply("Demand", betasd*(1-alpha));
							TempNode.Multiply("PeakPower", betasp*(1-alpha));
					}	else	{
	...
	for (unsigned int k = 0; k < ListArcs.size(); ++k) {
		...
		if ((ListArcs[k].Get("FromStep") == "") && (ListArcs[k].Get("ToStep") == "")) {
			...
			while ((TempStep <= SLength) && (TempToStep <= SLength)) {
				...
				if (dollar_factor != 1) {
					if ((TempStep[0] == SLength[0])) {// End Effects
						if (useEndDualE)
							TempArc.Multiply("OpCost", dollar_factor/(1-factor));
						if (useEndPrimE)
							TempArc.Multiply("OpCost", dollar_factor/(1-factor*Loadgrowth));
					}
					else
				...
				if (useEndSalvg) { // End Effect April 18 2013
					// Need to adjust for investment costs at the end of the simulation period- salvage value
				...
				if (dollar_factor != 1) {
					if ((TempArc.Get("Distance") != "X") && (Str2Step(TempArc.Get("LifeSpan"))[0] < 55)){ // NEW! July 03 2012 - Except for HSR infra, which needs to be multiplied by Distance
						double distance = TempArc.GetDouble("Distance");
						dollar_factor = dollar_factor / distance; // changed transportation cost Dec 07 2011 - in $, not $/mile
					}
					if ((TempStep[0] == SLength[0])) {// End Effects
						if (useEndDualE)
							TempArc.Multiply("InvCost", dollar_factor/(1-factor));
						if (useEndPrimE)
							TempArc.Multiply("InvCost", dollar_factor/(1-factor*Loadgrowth));
					}
					else
				...
				if (useEndDualE && (TempStep[0] == SLength[0])) { 
					...
					if (TempArc.Get("InvMax") != "Inf") // June 23 2014 - when invmax "inf"- end effects
						TempArc.Multiply("InvMax", (1+betas)*(1-factor));
				}
	...
	afile << "ROWS" << endl;
	afile << " N obj" << endl;
	afile << " E HTobj" << endl; // End effects - horizon time period objective function May 3 2013
	for (int i = 0; i <= nyears; ++i) {
		myfile[i] << "NAME" << endl;
		myfile[i] << "ROWS" << endl;
	...
	// End Effects - Objective function computation - May 03 2013
	afile << "    HTobjvar	HTobj	-1" << endl;
	// Save index for end effect obj. fn. metrics
	IdxHo.Add(0, 0, 1, "HTobj");
	...
	IdxHo.WriteFile("prepdata/idx_ho.csv"); // end effects
